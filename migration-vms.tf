...
resource "azurerm_network_interface" "migration_vms_2" {
  for_each            = var.migration_vms_2
  name                = "${each.key}-nic"
  location            = azurerm_resource_group.darts_migration_resource_group[0].location
  resource_group_name = azurerm_resource_group.darts_migration_resource_group[0].name
  tags                = var.common_tags
...
resource "azurerm_managed_disk" "migration_vms2_data" {
  for_each             = var.migration_vms_2
  name                 = "${each.key}-datadisk"
  location             = azurerm_resource_group.darts_migration_resource_group[0].location
  resource_group_name  = azurerm_resource_group.darts_migration_resource_group[0].name
...
resource "azurerm_windows_virtual_machine" "migration_windows_server" {
  for_each              = var.migration_vms_2
  name                  = each.key
  location              = azurerm_resource_group.darts_migration_resource_group[0].location
  resource_group_name   = azurerm_resource_group.darts_migration_resource_group[0].name
...
resource "azurerm_virtual_machine_extension" "migration_windows_joinad2" {
  for_each             = var.migration_vms_2
  name                 = "${each.key}-joinad"
  virtual_machine_id   = azurerm_windows_virtual_machine.migration_windows[each.key].id
  publisher            = "Microsoft.Compute"
...
resource "azurerm_virtual_machine_data_disk_attachment" "migration_vms_datadisk_2" {
  for_each           = var.migration_vms_2
  managed_disk_id    = azurerm_managed_disk.migration_vms_data[each.key].id
  virtual_machine_id = azurerm_windows_virtual_machine.migration_windows[each.key].id
  lun                = "10"
